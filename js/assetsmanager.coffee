class @AssetsManager extends Manager
  constructor:(app)->
    ...
    @get_item = "getAsset"
    @use_thumbnails = true
    @extensions = ["glb","obj","json","ttf","png","jpg","txt","csv","md","wasm","js"]
  ...
  loadWasmInitializer:(assetName,callback)->
    asset = @app.project.getAsset(assetName)
    if asset? and asset.ext is "js"
      script = document.createElement "script"
      script.src = asset.getURL()
      script.onload = callback
      script.onerror = (=> console.error "Failed to load JS initializer: #{assetName}")
      document.body.appendChild script
    else
      console.error "Asset not found or is not a JS file: #{assetName}"
